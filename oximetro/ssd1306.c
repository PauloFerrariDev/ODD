...
void ssd1306_printText(uint8_t x, uint8_t y, char *ptString) {
    ...
    while (*ptString != '\0') {
        ...
        if ((col + 6) > 127) {                                          // char will run off screen
            col=0;                                                      // set column to 0
            page++;                                                     // jump to next page

            if(page>7) page=0;

            ssd1306_setPosition(col, page);                                // send position change to oled
        }
...
void ssd1306_drawBitmapUTFPR(void) {
    ...
    const uint16_t size = sizeof bitmap_UTFPR / sizeof *bitmap_UTFPR;
    uint16_t i;
    for(i=0; i<size; i++){
        buffer[1]=bitmap_UTFPR[i];
        i2c_write(buffer, 2);
    }

} // end ssd1306_drawBitmapUTFPR

void ssd1306_drawBitmap(void) {
    buffer[0]=0x40;
    buffer[1]=0xff;

    ssd1306_setPosition(64, 3);
    i2c_write(buffer, 2);

} // end ssd1306_drawBitmap
